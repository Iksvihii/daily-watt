...
import { CreateImportJobRequest, ImportJob, SaveCredentialsRequest } from '../models/enedis.models';
...
export class EnedisService {
  ...
  saveCredentials(request: SaveCredentialsRequest): Observable<void> {
    return this.http.post<void>(`${environment.apiUrl}/api/enedis/credentials`, request);
  }

  createImportJob(request: CreateImportJobRequest): Observable<ImportJob> {
    return this.http.post<ImportJob>(`${environment.apiUrl}/api/enedis/import`, request);
  }

  getImportJob(id: string): Observable<ImportJob> {
    return this.http.get<ImportJob>(`${environment.apiUrl}/api/enedis/import/${id}`);
  }
